...
/**
 * Production OAuth MCP Server - Like Official Atlassian Implementation
 * Simplified but fully functional version
 */
class ProductionOAuthServer {
  ...
  setupRoutes() {
    ...
    this.app.get('/oauth/authorize', (req, res) => {
      ...
      const jiraUrl = process.env.JIRA_URL || 'https://codegenie.atlassian.net';
  ...
  async start() {
    return new Promise((resolve, reject) => {
      this.server = this.app.listen(this.port, () => {
        ...
        console.log(chalk.gray(`📡 Server: http://localhost:${this.port}`));
        console.log(chalk.gray(`🔐 OAuth: http://localhost:${this.port}/oauth/authorize`));
        console.log(chalk.gray(`💚 Health: http://localhost:${this.port}/health`));
        console.log(chalk.gray(`📋 Config: http://localhost:${this.port}/.well-known/mcp-configuration`));
